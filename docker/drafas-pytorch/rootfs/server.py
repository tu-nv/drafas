...
model.load_state_dict(torch.load(f"{BASE_DIR}/resnet152_weights.pth"))  # Load local weights
...
imagenet_classes = {idx: entry for (idx, entry) in enumerate(labels)}

def preprocess_image(image: Image.Image) -> torch.Tensor:
    """
    Preprocess a single image to a tensor suitable for inference.
    """
    # Convert to NumPy array
    image_array = np.array(image)
    image_tensor = torch.as_tensor(image_array, device=device).float() / 255.0
...
async def classify_image(file: UploadFile = File(...)):
    try:
        ...
        image_tensor = torch.as_tensor(np.array(image), device=device).float() / 255.0
        image_tensor = image_tensor.permute(2, 0, 1)
        ...
        predicted_class = imagenet_classes[predicted_class_idx.item()]
...
async def classify_batch(files: list[UploadFile] = File(...)):
    try:
        ...
        return JSONResponse(content={"predicted_classes": predicted_classes})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)
