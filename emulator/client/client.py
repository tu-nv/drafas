...
async def post_request(delay_before_request, request_data, session: aiohttp.ClientSession, timeout=60):
    ...
    try:
        ...
        async with results_lock:
            results.append([time.time(), processing_time])
...
async def cal_results():
    async with results_lock:
        if len(results) > 0:
            processing_times = [x[1] for x in results]
            average_time = sum(processing_times) / len(processing_times)
            min_time = min(processing_times)
            max_time = max(processing_times)
            print(f"Number of success requests: {len(processing_times)}")
...
async def get_stats():
    ...
    async with results_lock:
        if len(results) > 0:
            # only take results of last 15 secs
            recent_proc_times = [x[1] for x in results if (now - x[0]) * args.speedup <= 15]
